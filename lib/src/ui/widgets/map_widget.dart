...
class _MapWidgetState extends State<MapWidget> with TickerProviderStateMixin {
              ...
              children: [
                FlutterMap(
                  ...
                  layers: [
                    ...
                    MarkerLayerOptions(markers: createStopsMarkers()),
                    MarkerLayerOptions(markers: [
                      Marker(
                          ...
                          builder: (_) => const Center(
                                child: Image(
                                  image:
                                      AssetImage('assets/images/bus_point.png'),
                                  height: 60,
                                ),
                              ))
                    ])
                  ...
                  child: GestureDetector(
                    onTap: () {
                      MapUtils().animatedMapMove(
                          userLocation, 16.5, this, widget.mapController);
                    },
                    child: Container(
  ...
  List<Marker> createStopsMarkers() {
    List<Stop> stops = (Provider.of<StopBloc>(context, listen: false).state
            as StopsLoadedState)
        .stops;
...
class _TravelBuilderState extends State<TravelBuilder> {
      ...
      builder: (context, stopstate) {
        return BlocBuilder<BusBloc, BusState>(
          builder: (context, bustate) {
            return BlocBuilder<UserLocationBloc, UserLocationState>(
              builder: (context, userLocationState) {
                var userLocationStateLoaded =
                    userLocationState as UserLocationLoaded;
                LatLng busLocation = LatLng(userLocationStateLoaded.latitude,
                    userLocationStateLoaded.longitude);
                Bus bus = (bustate as BusesLoadedState)
                    .buses
                    .firstWhere((element) => element.id == widget.busId);
                Stop actualNextStop = (stopstate as StopsLoadedState)
                    .stops
                    .firstWhere((e) => e.id == bus.nextStop);
                MapUtils().animatedMapMove(busLocation, 16.5,
                    widget.tickerProvider, widget.mapController);

                if (Matrix().calculateDistanceInMeters(
                        busLocation,
                        LatLng(actualNextStop.latitude,
                            actualNextStop.longitude)) <
                    50) {
  ...
  String getNextStop(String busId) {
    ...
    Bus bus = (bustate as BusesLoadedState)
        .buses
        .firstWhere((element) => element.id == busId);
